...
//  приватная функция (которая не експортируется)
//  для подсчета размера картинки
function bytesToSize(bytes) {
  var sizes = ['Bytes', 'KB', 'MB', 'GB', 'TB'];
  if (bytes == 0) return '0 Byte';
  var i = parseInt(Math.floor(Math.log(bytes) / Math.log(1024)));
  return Math.round(bytes / Math.pow(1024, i)) + ' ' + sizes[i];
} //  ф-я для упрощения создания елементов


var elementShow = function elementShow(tag) {
  ...
  return node;
}; //  функция загрузки файлов (изображений) 
...
function Upload(selector) {
  ...
  // второй параметр принимает массив обектов (фото)
  function noop() {}

  var server = (_options$onUpload = options.onUpload) !== null && _options$onUpload !== void 0 ? _options$onUpload : noop;
  var files = [];
  var input = document.querySelector(selector);
  var preview = elementShow('div', ['preview']);
  preview.classList.add('preview'); //создание динамической кнопки

  var OpenButton = elementShow('button', ['btn', 'green'], "Open images");
  var upload = elementShow('button', ['btn', 'primary'], "Open images");
  upload.style.display = "none"; // проверка выбора нескольких файлов (фото)

  if (options.multi) {
    input.setAttribute('multiple', true);
  } // проверка файла на наличие нужного типа


  if (options.accept && Array.isArray(options.accept)) {
    input.setAttribute('accept', options.accept.join(','));
  } //insertAdjacentElement - всатвка елемента (JS) у DOM


  input.insertAdjacentElement('afterend', preview);
  input.insertAdjacentElement('afterend', upload);
  input.insertAdjacentElement('afterend', OpenButton); // todo создание  события для открытия окна загрузки
  // обработка инпут

  var trigger = function trigger() {
    return input.click();
  }; // реакия самой кнопки и вызов события для инпута


  OpenButton.addEventListener('click', trigger);

  var catchFiles = function catchFiles(event) {
    if (event.target.files === null) {
      return;
    } // по умолчанию files не является массивом
    // для этого нужно вызвать метод from у класса Array


    files = Array.from(event.target.files); //  создание превью картинки
    // удаление ранее загруженных файлов

    preview.innerHTML = '';
    preview.insertAdjacentHTML('afterend', "Count of pictures: ".concat(files.length));
    files.forEach(function (file) {
      ...
      var reader = new FileReader(); //  создание функционала предварительной подгрузки фото

      reader.onload = function (evnImg) {
        var srcImages = evnImg.target.result; // вставка фото на экран

        preview.insertAdjacentHTML('afterbegin', "\n    <div class=\"preview-image\">\n     <div class=\"preview-remove\" data-name=\"".concat(file.name, "\">&times</div>\n      <img src=\"").concat(srcImages, "\" alt=\"").concat(file.name, "\"/>\n       <div class=\"preview-info\">\n        <span>").concat(file.name, "</span>\n        <span>").concat(bytesToSize(file.size), "</span>\n       </div>\n    </div>\n   "));
      };
  ...
  //чтобы отловить событие по отдельной картинке
  // нужно обращатся к её data-атрибуту


  var removeHadler = function removeHadler(event) {
  ...
  var clearInfoPreview = function clearInfoPreview(el) {
    el.style.bottom = "5px";
    el.innerHTML = "<div class=\"progress\"></div>";
  }; //


  var ServerUploading = function ServerUploading() {
    preview.querySelectorAll('.preview-remove').forEach(function (e) {
      return e.remove();
    });
    var previewInfo = preview.querySelectorAll('.preview-info');
    previewInfo.forEach(clearInfoPreview);
    server(files, previewInfo);
  }; //  вызов ф-й


  preview.addEventListener('click', removeHadler);
  input.addEventListener('change', catchFiles);
  upload.addEventListener('click', ServerUploading);
}
...
// todo подключение Firebase
var firebaseConfig = {
  apiKey: "AIzaSyCHXLmaASGMxi5lOlXPaP48gWlgrXf_I70",
  authDomain: "uploading-images-cf9c0.firebaseapp.com",
  projectId: "uploading-images-cf9c0",
  storageBucket: "uploading-images-cf9c0.appspot.com",
  messagingSenderId: "42368233198",
  appId: "1:42368233198:web:06c5a1a48ac1d6024ecdff"
}; // Initialize Firebase

_app.default.initializeApp(firebaseConfig);

var storage = _app.default.storage(); // передаеи в качестве значения селектор ел-та
// который будет появлятся


(0, _upload.Upload)('#files', {
  multi: true,
  // указывает модификатор для загрузки нескольких фото
  accept: ['png', 'jpg', 'jpeg'],
  // multi и accept - атрибуты тега input
  // todo ф-я обработки (загрузки) файлов на сервер (firebase)
  onUpload: function onUpload(files, blocks) {
    // прокидование массива файлов и инфоблока 
    files.forEach(function (file, index) {
      var ref = storage.ref("images/".concat(file.name)); // сохранение файла в storage firebase

      var task = ref.put(file); // обработка загрузки файлов (к-во процентов)

      task.on('state_changed', function (snapshot) {
        var percent = (snapshot.bytesTransferred / snapshot.totalBytes * 100).toFixed(0);
        var progressBlock = blocks[index].querySelector('.progress'); //отображение прогресса загрузок

        progressBlock.textContent = percent + ' %';
